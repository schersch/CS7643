...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        tokenizer, num_max_bpe_tokens, task=None, aug=False
    ):
        index_files = self.get_index_files(split, task=task, aug=aug) #modified for hateful_memes
...
#-----------------------------------------------------------
# Begin original code contribution based on existing classes
#-----------------------------------------------------------
...
class HatefulMemesDataset(BaseDataset):
    def __init__(
        ...
        index_files = self.get_index_files(split, aug=aug)
        self.tokenizer = tokenizer
        self.num_max_bpe_tokens = num_max_bpe_tokens
        self.data_path = data_path
        items = []
    ...
    @staticmethod
    def get_index_files(split, aug=False):
        entity = ".entity" if aug else ""
        if split == "train":
            return (f"train{entity}.index.jsonl", )
        elif split == "val":
            return (f"dev_seen_unseen{entity}.index.jsonl", )
        elif split == "test":
            return (f"test_seen_unseen{entity}.index.jsonl", )
        else:
    ...
    @classmethod
    def make_dataset_index(cls, data_path, tokenizer, aug=False):
        def preprocess_json(json_file, tokenizer, index_file):
            items = []
            with open(json_file, mode="r", encoding="utf-8") as reader:
                for line in reader:
                    ...
                    desc_token_ids = []
                    if aug:
                        image_desc_words = " ".join([word for nestedlist in data["partition_description"] for word in nestedlist])
                        desc_tokens = tokenizer.tokenize(image_desc_words)
                        desc_token_ids = tokenizer.convert_tokens_to_ids(desc_tokens)
                    items.append({
                        "image_path": data["img"],
                        "text_segment": token_ids + desc_token_ids,
                        "label": data["label"],
                        "id": data["id"],
                    })
            _write_data_into_jsonl(items, index_file)
        ...
        preprocess_json(
            json_file=os.path.join(data_path, f"train{entity}.jsonl"), 
            tokenizer=tokenizer, index_file=os.path.join(data_path, cls.get_index_files("train", aug=aug)[0]), 
        )
        preprocess_json(
            json_file=os.path.join(data_path, f"dev_seen_unseen{entity}.jsonl"), 
            tokenizer=tokenizer, index_file=os.path.join(data_path, cls.get_index_files("val", aug=aug)[0]), 
        )
        preprocess_json(
            json_file=os.path.join(data_path, f"test_seen_unseen{entity}.jsonl"), 
            tokenizer=tokenizer, index_file=os.path.join(data_path, cls.get_index_files("test", aug=aug)[0]), 
        )
        
#-----------------------------------------------------------
# End original code contribution based on existing classes
#-----------------------------------------------------------
