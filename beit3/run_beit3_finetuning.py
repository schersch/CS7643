...
def get_args():
    ...
    parser.add_argument('--task', type=str, required=True, 
                        choices=['nlvr2', 'vqav2', 'flickr30k', 'coco_retrieval', 'coco_captioning', 'nocaps', 'imagenet', 'hateful_memes'], 
    ...
    #-----------------------------------------------------------
    # Begin original code contribution based on existing params
    #-----------------------------------------------------------
    # hateful memes parameters
    parser.add_argument('--data', type=str, default="unaug", help="Augmented (aug) or unaugmented (unaug) data")
    #-----------------------------------------------------------
    # End original code contribution based on existing params
    #-----------------------------------------------------------
...
def main(args, ds_init):
    ...
    if args.eval:
        ...
        #----------------------------------
        # Begin modified code from original
        #----------------------------------
        if args.task in ["nlvr2", "flickr30k", "coco_retrieval", "imagenet", "hateful_memes"]:
            eval_res = evaluate(data_loader_test, model, device, task_handler)
            ext_test_stats, task_key = eval_res[:2]
            if len(eval_res) == 3:
                with open("results/preds.json", "w") as outfile:
                    json.dump(eval_res[2], outfile)
        #--------------------------------
        # End modified code from original
        #--------------------------------
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            if args.task not in ["coco_captioning", "nocaps"]:
                test_stats, task_key, *_ = evaluate(data_loader_val, model, device, task_handler)
            else:
            ...
            print(f"Performance of the network on the {len(data_loader_val.dataset)} val images: {test_stats[task_key]:.3f}")
            if max_auroc < test_stats[task_key]:
                max_auroc = test_stats[task_key]
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch="best", model_ema=model_ema)

            print(f'Max performance: {max_auroc:.3f}%')
            if log_writer is not None:
