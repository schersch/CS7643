...
class HatefulMemesHandler(TaskHandler):
    def __init__(self) -> None:
        super().__init__()
    ...
    def eval_batch(self, model, image, language_tokens, padding_mask, label):
        ...
        auroc = roc_auc_score(label.tolist(), torch.softmax(logits, dim=-1)[:, 1].tolist())
        val_loss = self.criterion(input=logits, target=label)
        self.metric_logger.meters['auroc'].update(auroc.item(), n=batch_size)
        self.metric_logger.meters['acc'].update(acc.item(), n=batch_size)
        self.metric_logger.meters['loss'].update(val_loss.item(), n=batch_size)

        self.labels.extend(label.tolist())
        self.probs.extend(torch.softmax(logits, dim=-1)[:, 1].tolist())

    def after_eval(self, **kwargs):
        print('* Acc {acc.global_avg:.3f}'.format(acc=self.metric_logger.acc))
        print('* AUROC {auroc.global_avg:.3f}'.format(auroc=self.metric_logger.auroc))
        ...
        return {k: meter.global_avg for k, meter in self.metric_logger.meters.items()}, "auroc", eval_results
...
def get_handler(args):
    if args.task == "nlvr2":
        return NLVR2Handler()
    elif args.task in ("hateful_memes"):
        return HatefulMemesHandler()
